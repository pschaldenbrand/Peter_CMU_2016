...
def main():
	...
	#for fn in os.listdir('./datasets'):
		...
		allLines = open('megaData1.txt','r').readlines()
		...
		if (True):		#getting the field indexes
			...
			probSetIndex = fields.index('Level (ProblemSet)')
			timeIndex = fields.index('CF (tool_event_time)')
		...
		for i in range(1, len(allLines)):
			...
			problemSet = tokens[probSetIndex]
...
#				#printInstance(prevEq, currEqL+'='+currEqR, outcome, sid)
				printLots(prevEq,currEqL+'='+currEqR,outcome,problemSet,sid,time)
...
def printLots(s1,s2,s3,s4,s5,s6):
